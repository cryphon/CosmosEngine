...
void UI::render() {
    ...
    try {
        ...
        if (selected_index >= 0 && selected_index < objects.size()) {
            scene::SceneObject& obj = objects[selected_index];
            ...
            // Shader selection
            std::vector<std::string> shader_keys = assets::ShaderLibrary::get_keys();
            std::string current_shader_name = assets::ShaderLibrary::get_name(obj.material->shader);
            ...
            if (ImGui::BeginCombo("Shader", current_shader_name.c_str())) {
                for (const auto& name : shader_keys) {
                    ...
                    if (ImGui::Selectable(name.c_str(), selected)) {
                        ...
                        obj.material->shader = assets::ShaderLibrary::get(name);
                    }
                    if (selected)
    ...
    if (ImGui::Begin("Material Editor")) {
        if (selected_index >= 0) {
            scene::SceneObject& obj = scene->get_objects()[selected_index];
            auto mat = std::dynamic_pointer_cast<render::PBRMaterial>(obj.material);
            if (mat) {
...
void UI::show_shader_settings_popup(scene::SceneObject& obj) {
    if (ImGui::BeginPopup("ShaderSettingsPopup")) {
        std::string shader_name = assets::ShaderLibrary::get_name(obj.material->shader);
...
bool UI::EditTextureSlot(const char* label, bool& use_map, std::shared_ptr<render::Texture>& texture, GLenum texture_target) {
    bool changed = false;
